...
public class partialRSA
{
	public static void main(String[] args)
	{
		...
		ArrayList<BigInteger> decryptText = decrypt(cyphText, d, n); // Write this method
		ArrayList<String> decipherToString = plainToStringBlocks(decryptText); //Turns the text into size 200 blocks
		stringToFile(decipherToString);
	}

	// Takes the ArrayList of Strings and converts to a single string and writes it to a file
	public static void stringToFile(ArrayList<String> strings)
	{
		String result = "";
	...
	//Takes an ArrayList of BigIntegers and converts to ArrayList of String blocks, length 200
	public static ArrayList<String> plainToStringBlocks(ArrayList<BigInteger> biBlocks)
	{
		...
		Iterator<BigInteger> it = biBlocks.iterator();
		...
		while(it.hasNext())
		{
			...
			{
				ctemp = unPaddedAscii(temp.substring(i, i+3));
				result += Character.toString(ctemp);
			}
	...
	//Reverse the toPaddedAscii method
	public static char unPaddedAscii(String letter)
	{
		int ascii = Integer.parseInt(letter);
		char c = (char)(ascii-100);

		return c;
	}

	// Takes an ArrayList of BigIntegers and decrypts each block using the private key.
	public static ArrayList<BigInteger> decrypt(ArrayList<BigInteger> ciBlocks, BigInteger d, BigInteger phi)
	{
		ArrayList<BigInteger> plaintext = new ArrayList<BigInteger>();
		BigInteger temp;
		Iterator<BigInteger> it = ciBlocks.iterator();

		while(it.hasNext())
		{
			temp = it.next();
			plaintext.add(temp.modPow(d, phi));
		}

		return plaintext;
	}

	// Takes an ArrayList of BigInts and encrypts each block using the public key.
	public static ArrayList<BigInteger> encrypt(ArrayList<BigInteger> biBlocks, BigInteger e, BigInteger n)
	{
		ArrayList<BigInteger> cipherBlocks = new ArrayList<BigInteger>();
		BigInteger temp;
		Iterator<BigInteger> it = biBlocks.iterator();
		...
		{
			temp = it.next();
			cipherBlocks.add(temp.modPow(e, n));
		}
